...
- Project JSON uses schema 4, with nullable `collection.maxSupply` (`null` means unlimited), `reveal.mode` (`shuffle`, `offset`, `token-hash`) and `reveal.unrevealedUri` (empty means no placeholder). Earlier schemas are not migrated; stored records from them are isolated by workspace recovery with the original preserved. Metadata references must use lowercase `https://`, `ipfs://` or `ar://`. Business validation reports missing or incompatible options separately, including warnings for selected payment policies the generated contracts do not implement.
...
- A successful browser compilation means that the supplied Solidity sources compiled with the reported compiler and settings. The client also checks EIP-170 runtime and EIP-3860 creation-code sizes and reports oversized contracts as not deployable. It does not prove correct economics, completed NFT integration, or deployment readiness.
...
- ERC-1155 loot: one weighted reward per opening; explicit token IDs; total-unit supply including premint and outstanding reservations. Premint goes to the first configured item ID. `ITEMS_ROOT` is a SHA-256 Merkle commitment to every (index, token ID, URI) item. The constructor stores leading items within a 256-storage-slot budget; `config/item-registration.json` carries proofs for the rest, which anyone can submit to `registerItems`. Openings revert with `ItemsNotRegistered` until every item is stored. Consumers keep cumulative weights and token IDs as packed code constants and select rewards by binary search. `_authorizeOpen` is the entitlement/price seam and admits every caller as generated. A settled expired attempt releases its reservation; retry reacquires capacity. There is no separate lifetime opening quota. `openTo` accepts a separate NFT recipient; the RNG refund recipient remains the caller. After acceptance, only the original requester may update an undelivered NFT recipient with `setDeliveryRecipient`, preserving the request, token, word and reserved supply. `deliver(requestId)` always uses the recorded recipient. Existing-project adapters leave inventory limits and eligibility to the host integration.
- ERC-721 reveal: sequential IDs, optional supply cap and owner-controlled distribution. `mintPremint(quantity)` distributes the fixed allocation before ordinary minting; `closeMint()` can close below a configured cap. Minting can continue during a pending reveal. A request freezes the next minted range; expired retries retain it. `finalizeReveal(requestId)` applies that accepted result once before another batch begins and emits ERC-4906 `BatchMetadataUpdate`. Unrevealed tokens return the fixed `unrevealedUri`, or an empty URI when none is configured.
...
- New collections require metadata URIs before Solidity generation because their URI rules are fixed at generation. Incomplete metadata produces a planning bundle; existing-project adapters retain metadata as integration work.
- New-project exports include `foundry.toml` with the pinned compiler settings and remappings. Generated agent instructions include a deployment sequence and client guidance from the pinned SDK: off-chain fee quoting with `quoteRequestFee`/`quoteFeeAt`, request tracking, `refundRequest`, `retryCallback`, `retryRefundCallback`, refund-credit withdrawal, error decoding and RPC constraints.

`tests/evm-lifecycle.test.ts` runs generated contracts inside a local Hardhat EVM using a coordinator lifecycle double. It covers callback and notification failures, retries, reservation ordering, rejecting NFT recipients, optional caps, staged premint, minting during reveals, frozen batch boundaries, all reveal modes, a 256-item loot table with proof registration, an `_authorizeOpen` override and ERC-4906 reveal events. It does not claim to test real onchain VRF acceptance or a live deployment.
